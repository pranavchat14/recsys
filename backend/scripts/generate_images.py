...
def draw_pattern(draw, shape, primary_color, secondary_color, width, height):
    if shape == 'rectangle':
        draw.rectangle([20, 20, width-20, height-20], fill=primary_color, outline=secondary_color)
    elif shape == 'ellipse':
        draw.ellipse([20, 20, width-20, height-20], fill=primary_color, outline=secondary_color)
    elif shape == 'triangle':
        draw.polygon([width/2, 20, 20, height-20, width-20, height-20], fill=primary_color, outline=secondary_color)
...
def create_feature_image(title, width=400, height=200):
    ...
    secondary_shape = random.choice([shape for shape in shapes if shape != primary_shape])
    primary_color = random.choice(colors)
    secondary_color = random.choice([color for color in colors if color != primary_color])
    ...
    draw_pattern(overlay_draw, secondary_shape, secondary_color, primary_color, width//2, height//2)
